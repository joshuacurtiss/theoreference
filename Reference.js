class Reference {
    ...
    set pars(pars) {
        ...
        if( pars && Array.isArray(pars) ) this._pars=pars;
            ...
            for( var p of pars.split(",") ) {
                ...
                else this._pars.push(parseInt(p));
            }
        }
    }
    parsToString() {
    ...
    valid() {
        return this.publication && this.chapter && (this.pars.length>0);
    }
    toString() {
        return `${this.publication.name} ${this.chapter}:${this.parsToString()}`;
    }
    toAbbrevString() {
        return `${this.publication.symbol}${this.chapter}:${this.parsToString()}`.replace(/\s/g,"");
    }
}
